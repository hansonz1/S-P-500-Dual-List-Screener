...
1. **Scrapes** the current S&P 500 constituent list from Wikipedia (so the
   universe never goes stale).
2. **Downloads** one year of 1-hour OHLC bars for every constituent plus the
   ^GSPC benchmark from Yahoo Finance.
3. **Resamples** 1-hour bars into 2-hour bars (7 hourly bars per session → 4
   two-hour bars, never pairing across a day boundary).
4. **Screens** every stock against two independent signal sets (below).
5. **Renders** a PDF report + machine-readable JSON, optionally adds an
...
|---|---|
| `screener/data.py` | Constituent scraping, 1h download, 2h resampling |
| `screener/indicators.py` | EMA, Bollinger Bands, O(n) rolling beta |
| `screener/screen.py` | The two signal rules |
| `screener/report.py` | PDF rendering (reportlab) |
| `screener/commentary.py` | Optional LLM commentary (skipped without key) |
| `run.py` | Orchestration, chunked downloads, error isolation |
...
See [`output/`](output/) for daily JSON + PDF archives. A typical day yields
